...
import { getPurchaseGoal, changePurchaseGoalStatus, deletePurchaseGoal } from '../../api/api';
...
const GroupInfo = () => {
  ...
  const handleStatusChange = async () => {
    try {
      await changePurchaseGoalStatus(id);
      setModalMessage('Purchase goal status changed successfully!');
      ...
      setModalMessage(`Error changing purchase goal status: ${err.message}`);
      setModalType('error');
      setIsModalOpen(true);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      await deletePurchaseGoal(id);
      setModalMessage('Purchase goal deleted successfully!');
      setModalType('success');
      setIsModalOpen(true);
      navigate('/groups');
    } catch (err) {
      setModalMessage(`Error deleting purchase goal: ${err.message}`);
      setModalType('error');
      setIsModalOpen(true);
    }
  };
      ...
      <div className="p-4">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex items-center">
              <button className="bg-blue-500 text-white px-4 py-2 rounded-md mr-2" onClick={() => navigate(`/create-group`)}>
                Edit
              </button>
          ...
          <div className="mt-6">
            <button className="bg-yellow-500 text-white px-4 py-2 rounded-md" onClick={handleStatusChange}>
              Change Status
            </button>
          </div>
        </div>
